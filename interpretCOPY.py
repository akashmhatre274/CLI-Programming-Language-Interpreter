...
def interpretDTREE(d) :
    ...
    # ["proc", ID, ILIST, [], CLIST]
        ...
        # if d[2] == 'nil':
        #     declare(active_ns, d[1],d[2])
        # else:
        #     handle = allocateNS()
        #     declare(active_ns, d[1], handle)
        #     
        #     sublist = d[2]
        #     sublist = sublist[1]
        #     sublist = sublist[1]
        #     #print('sublist[2] d[2]', sublist)
        #     print('sublist[0]',sublist[0])
        #     print('sublist[1', sublist[1])
        #     interpretDLIST(sublist)


        #handle = allocateNS()
        # heap[handle]['parentns'] = active_ns
        #declare(active_ns, d[1], topHandle()) 
        val = interpretETREE(d[2])
        print("Val",val)
        declare(active_ns, d[1], val) 
        # active_ns = activeNS()
...
def interpretTTREE(ttree):
    ...
    if ttree[0] == 'struct':
        ...
        ans = interpretLTREE(ttree[1])
        anslookup = lookup(ans[0], ans[1])
        #print("ans outside if",anslookup)
        if lookup(anslookup,'type') == 'class':
            body = lookup(anslookup,'body')
            #print("ans[0]",ans[0])
            pushHandle(anslookup)
...
def interpretETREE(etree) :
    ...
    #elif etree == "nil":
    elif isinstance(etree,str) and (etree == "nil") : 
        ans = 'nil'
    elif etree[0] == "new":
